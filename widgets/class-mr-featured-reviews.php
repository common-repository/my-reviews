 ...
 * @return void
 */
function mr_register_featured_reviews_widget() {
	register_widget( 'MR_Featured_Reviews' );
}
add_action( 'widgets_init', 'mr_register_featured_reviews_widget' );

/**
 * Twitter feeds widget
 */
class MR_Featured_Reviews extends WP_Widget {
	
	const CACHE_KEY = 'mr_featured_reviews';
	...
	public function __construct() {
		...
		parent::__construct( false, __( 'My Reviews - Featured Reviews', 'my-reviews' ), array( 'description' => __( 'Displays featured reviews', 'my-reviews' ) ) );
	}
	...
	public function update( $new_instance, $old_instance ) {
		delete_transient( MR_Featured_Reviews::CACHE_KEY );
	...
	public function widget( $args, $instance ) {
		...
		$cached_html = get_transient( MR_Featured_Reviews::CACHE_KEY );
		...
		if ( empty( $cached_html ) ) {
			...
			set_transient( MR_Featured_Reviews::CACHE_KEY, $cached_html, HOUR_IN_SECONDS );
		}
